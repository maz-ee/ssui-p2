...
export class Row extends Group {
  ...
  public get hJustification() {
    return this._hJustification;
  }
  ...
  protected override _doLocalSizing(): void {
    ...
    for (let i = 0; i < this.children.length; i++) {
      ...
      w = SizeConfig.add(w, child.wConfig);
      h = SizeConfig.maximum(child.hConfig, h);
    }
    this._wConfig = w;
    this._hConfig = h;
  }
  ...
  protected _adjustChildren() {
    ...
    if (excess >= 0) {
      ...
      // negative excess (AKA shortfall) case
  ...
  protected _measureChildren(): [number, number, number] {
    ...
    for (let child of this.children) {
      if (child instanceof Spring) {
        ...
        natSum = natSum + child.wConfig.nat;
        availCompr = availCompr + (child.wConfig.nat - child.wConfig.min);
      }
    }
  ...
  protected _expandChildSprings(excess: number, numSprings: number): void {
    ...
    for (let child of this.children) {
      if (child instanceof Spring) child.w = expspace;
    }
  }
  ...
  protected _compressChildren(
    ...
    for (let child of this.children) {
      //=== YOUR CODE HERE ===
      let frac = (child.w - child.minW) / availCompr;
      child.w = child.w - frac / shortfall;
    }
  }
